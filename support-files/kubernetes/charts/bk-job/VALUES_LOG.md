...
```yaml
analysisConfig:
  # 审批相关配置，带审批的执行类接口（fast_execute_script_with_approval等）依赖此处的渠道配置
  approval:
    # 审批任务TTL，单位小时。TTL越长，参数快照在库中驻留越久、动态分组目标的漂移窗口越大
    ttlHours: 8
    # 调用方未指定审批渠道时使用的默认渠道，当前支持的取值：IMATE
    defaultChannel: IMATE
    # 审批任务记录的保留天数。审批记录是安全审计凭据，且参数快照含加密后的敏感数据，保留期越长数据面越大
    maxKeepDays: 30
    # "疑似秒批"监控指标的阈值，单位毫秒：审批通过时刻与发起时刻之差小于该值即计数
    fastApproveThresholdMillis: 5000
    # 各审批渠道的配置。渠道地址与appCode一律由服务端配置决定，调用方只能传渠道枚举值
    channels:
      # IMate审批渠道
      imate:
        # 渠道开放接口的根地址，回查审批结论时使用。留空则不注册IMate渠道，带审批的接口会返回"渠道不可用"
        url: ""
        # 该渠道取审批内容时使用的appCode，多个用英文逗号分隔。取内容接口据此校验"调用方属于该任务指派渠道的应用"，
        # 留空或配错会导致审批渠道取不到审批内容。这里是蓝鲸的appCode，与下面openApi中IMate颁发的凭证不是一回事
        appCode: ""
        # 作业平台回查IMate审批结论时使用的凭证。该凭证由IMate颁发，与蓝鲸的appCode/appSecret是两套东西，不要混用
        openApi:
          # IMate侧为作业平台分配的应用ID，以x-app-id请求头发出
          appId: ""
          # IMate侧为作业平台分配的应用密钥，以x-secret请求头发出
          secret: ""
        # Mock渠道配置，仅用于IMate回查接口就绪之前的自测
        mock:
          # 是否启用Mock渠道，默认不启用。启用后审批结果不再向真实渠道回查，因此在生产环境
          # （job.profile含prod）下启用会直接导致job-analysis启动失败。
          # 该开关决定Mock渠道Bean是否注册，只能在启动前就位，改动后必须重启job-analysis
          enabled: false
          # 视为"审批通过"的ID列表：与审批任务ID(approval_task_id)或审批单据ID(approval_ticket_id)
          # 任一相等即算通过，其余一律视为未通过。该项随可热刷新的配置下发，改动无需重启job-analysis
          approvedIds: []
```

2. 新增滚动执行并行错峰模式配置（最大批次总数、错峰调度器消费者线程数）
```yaml
executeConfig:
  # 滚动执行并行错峰模式配置
  rolling:
    scatter:
      # 并行错峰模式下允许的最大批次总数，超出则校验拒绝，防止并行 GSE 任务规模失控，默认200
      maxBatch: 200
      # 并行错峰批次下发调度器的消费者线程数，默认3
      workerNum: 3
```
