...
BEGIN
  ...
  WHILE fromId <= maxId DO
    ...
    UPDATE gse_file_execute_obj_task t1
    INNER JOIN (
      SELECT id, task_instance_id 
      FROM step_instance 
      WHERE id BETWEEN fromId AND endId)
    ...
    UPDATE gse_script_execute_obj_task t1
    INNER JOIN (
      SELECT id, task_instance_id 
      FROM step_instance 
      WHERE id BETWEEN fromId AND endId)
...
BEGIN
  ...
  WHILE fromId <= maxId DO
    ...
    INNER JOIN (
      SELECT id,app_id,create_time 
      FROM task_instance 
      WHERE id BETWEEN fromId AND endId)
    AS tmp ON t1.task_instance_id = tmp.id 
    SET t1.app_id = tmp.app_id, t1.job_create_time = tmp.create_time
    WHERE t1.app_id = 0;
