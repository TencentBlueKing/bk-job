...
本工具用于将老版本（3.11.x）的 BK-JOB 业务数据迁移到**启用了多租户**的 3.12.x 版本的 BK-JOB。
...
- ✅ **bk-cmdb（蓝鲸配置平台）已完成迁移**，目标环境的 `job_manage.application` 表中已有业务信息
...
> **⚠️ 重要：为什么需要先完成 bk-cmdb 迁移？**
> 
> 迁移工具需要根据 `bk_scope_id` 查询目标环境的 `app_id`（因为源环境和目标环境的 `app_id` 可能不同）。这个映射关系存储在目标环境的 `job_manage.application` 表中，该表的数据来源于 bk-cmdb 的同步。如果目标环境没有业务信息，迁移将会失败。
...
> ```
>
> 此脚本支持重复执行：设置前会比较当前偏移量与目标偏移量，若当前偏移量 >= 目标偏移量则跳过该表并打印警告日志。
...
> **说明**：此脚本支持重复执行。设置前会比较当前偏移量与目标偏移量：
> - 若当前偏移量 < 目标偏移量：执行设置
> - 若当前偏移量 >= 目标偏移量：跳过该表并打印 `[WARN]` 日志，提示检查是业务正常增长还是已设置过偏移量
...
1. **设置自增偏移量**：迁移前必须先运行 `tool_set_auto_increment_offset.sh`。脚本支持重复执行，会自动跳过当前偏移量已达到目标值的表
