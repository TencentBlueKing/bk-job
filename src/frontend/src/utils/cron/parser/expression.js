...
const CronDate = require('./date')

// Get Number.isNaN or the polyfill
const safeIsNaN = require('is-nan')
...
CronExpression._parseField = function _parseField (field, value, constraints) {
    ...
    switch (field) {
        case 'month':
        case 'dayOfWeek':
            const aliases = CronExpression.aliases[field]
   ...
   */
    function parseSequence (val) {
        ...
        function handleResult (result) {
            let max = stack.length > 0 ? Math.max.apply(Math, stack) : -1
...
CronExpression.prototype._findSchedule = function _findSchedule (reverse) {
   ...
   */
    ...
    const currentDate = new CronDate(this._currentDate, this._tz)
    ...
    while (stepCount < LOOP_LIMIT) {
        ...
        const dayOfMonthMatch = matchSchedule(currentDate.getDate(), this._fields.dayOfMonth)
        const dayOfWeekMatch = matchSchedule(currentDate.getDay(), this._fields.dayOfWeek)

        const isDayOfMonthWildcardMatch = isWildcardRange(this._fields.dayOfMonth, CronExpression.constraints[3])
        const isDayOfWeekWildcardMatch = isWildcardRange(this._fields.dayOfWeek, CronExpression.constraints[5])
...
CronExpression.prototype.next = function next () {
    const schedule = this._findSchedule()
...
CronExpression.prototype.prev = function prev () {
    const schedule = this._findSchedule(true)
...
CronExpression.parse = function parse (expression, options, callback) {
    ...
    function parse (expression, options) {
        ...
        for (let i = 0, c = CronExpression.map.length; i < c; ++i) {
            const field = CronExpression.map[i] // Field name
            const value = atoms[i]
            ...
            try {
                ...
                //     )
                // } else {
                const val = field === 'dayOfWeek' ? parseNthDay(value) : value
        ...
        if (mappedFields.month.length === 1) {
            const daysInMonth = CronExpression.daysInMonth[mappedFields.month[0] - 1]
