...
| 参数 | 必填 | 类型 | 说明 |
|------|------|------|------|
| `app_name` | ✅ | string | APM 应用名称（从 Prompt 的「环境参数」部分获取 `<app_name>` 的实际值） |
| `bk_biz_id` | ✅ | string | 蓝鲸业务 ID（从 Prompt 的「环境参数」部分获取 `<bk_biz_id>` 的实际值） |
...
```json
{
  ...
  "arguments": "{\"body_param\":{\"app_name\":\"<app_name>\",\"bk_biz_id\":\"<bk_biz_id>\",\"start_time\":\"{根据上下文确定}\",\"end_time\":\"{根据上下文确定}\",\"filters\":[{\"key\":\"trace_id\",\"operator\":\"equal\",\"value\":[\"{trace_id}\"]}],\"limit\":\"10\",\"offset\":\"0\"}}"
}
```
