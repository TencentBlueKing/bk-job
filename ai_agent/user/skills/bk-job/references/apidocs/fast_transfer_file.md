...
> **技能只支持服务器文件（file_type=1）与本地文件（file_type=2）两种源文件类型。** 接口本身还有第三方文件源（`file_type=3`，如 COS、制品库仓库），但相关接口尚未提供，技能**不支持**、脚本会拒绝，因此本文不再列出它的字段；引导用户选来源时也**只给这两项**。
...
| file_type        | int    | 否  | 文件源类型。**只能填** `1`（服务器文件）或 `2`（本地文件）；不传默认为 `1`。其余取值脚本会拒绝                                                   |
