#!/bin/bash
PROJECT_NAME=job
SERVICE_NAME=job-gateway-assemble
...
SHELL_FILE_NAME=${0##*/}
JAR_FILE=${SERVICE_HOME}/job-gateway.jar
...
function startup() {
	...
	JAVA_OPTS="$JAVA_OPTS -Dspring.config.additional-location=file://${JOB_CONF_HOME}/job-assemble/application-gateway-assemble.yml"
	JAVA_OPTS="$JAVA_OPTS -Dreactor.netty.http.server.accessLogEnabled=true"
    JAVA_OPTS="$JAVA_OPTS -DAPP_NAME=job-gateway-assemble"
