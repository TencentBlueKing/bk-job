...
function startup() {
    ...
    JAVA_OPTS="$JAVA_OPTS -Dspring.config.additional-location=file://${JOB_CONF_HOME}/application-config.yml"
	JAVA_OPTS="$JAVA_OPTS -DBK_JOB_CONFIG_DIR=${JOB_CONF_HOME}"

    if [ -n "${SPRING_PROFILE}" ];then
        JAVA_OPTS="$JAVA_OPTS -Dspring.profiles.active=${SPRING_PROFILE}"
	else
		JAVA_OPTS="$JAVA_OPTS -Dspring.profiles.active=native,prod"
    fi
