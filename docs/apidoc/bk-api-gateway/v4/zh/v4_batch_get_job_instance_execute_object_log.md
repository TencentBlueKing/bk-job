...
| host_id_list      | array  | 否  | 主机ID列表，最多100个。每个元素为long。优先级host_id_list > ip_list > container_id_list              |
| ip_list           | array  | 否  | 主机IP列表，最多100个。每个元素定义详见cloud_ip。优先级host_id_list > ip_list > container_id_list       |
| container_id_list | array  | 否  | 容器ID列表(BKCC中的容器UID)，最多100个。每个元素为long。优先级host_id_list > ip_list > container_id_list |
